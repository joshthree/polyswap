...
public class TimeLockTest {
	...
	public static String deploy() throws Exception {
		...
		polyswap.contracts.TimeLock t = polyswap.contracts.TimeLock.deploy(eth.getClient(), cred, new DefaultGasProvider(), cred.getAddress(), new BigInteger("20")).send();
		return t.getContractAddress();
	}
	
	public static polyswap.contracts.TimeLock load() {

		BigInteger privateKey = new BigInteger("89179811265080136650681266953673138167329384320295414412418920727415236104720");
		Credentials cred = Credentials.create(ECKeyPair.create(privateKey));
		
		return polyswap.contracts.TimeLock.load(eth.CONTRACT_ADDRESS, eth.getClient(), cred, new DefaultGasProvider());
	}
	
	public static void main(String[] arg) throws Exception {
		eth = new Ethereum(3);
	//	System.out.println(deploy());
//		polyswap.contracts.TimeLock t = load();
//		System.out.println(t.owner().send());
		
		//System.out.println(deploy());
		test();
		/*
		 * System.out.println("thisi aidjfa"); BigInteger privateKey = new BigInteger(
		 * "89179811265080136650681266953673138167329384320295414412418920727415236104720"
		 * ); Credentials cred = Credentials.create(ECKeyPair.create(privateKey));
		 * 
		 * SSSECDSA signer = new SSSECDSA(); System.out.println("Signer created");
		 * BigInteger m = eth.createDepositTransaction(cred.getAddress(),
		 * cred.getAddress(), new BigInteger("6000000000000"), new BigInteger("10"));
	...
	public static void test() throws Exception {
		...
		Credentials cred = Credentials.create(ECKeyPair.create(privateKey));
		//TimeLock t = TimeLock.deploy(eth.getClient(), cred, new DefaultGasProvider(), new BigInteger("10")).send();
		String contractAddress = "0x5f01de75e34817ca2f7efb81f4a360d8da5d6b67";
